...
set void ""
	...
	foreach_in_collection timing_point [get_attribute $wrt_path_collection points] {
		...
		if {![info exists celle_che_non_posso_cambiare($pin_name,ref)]} {
			...
			#set celle_che_non_posso_cambiare($pin_name,incL) [list $incLrtime]
		...
		set pin_name_temp [get_attribute [get_attribute $timing_point object] full_name]
                ...
                set cell_name  [get_attribute $pin_name ref_name]
                        ...
                        if {[regexp {\/[^Z].*} $pin_name_temp]} {
                                ...
                                set incLrtime [expr [get_attribute $timing_point arrival] - $starttime]
                                set starttime [get_attribute $timing_point arrival]

                                if {![info exists celle_che_posso_cambiare($pin_name,ref)]} {
                                        set celle_che_posso_cambiare($pin_name,ref) $cell_name
                                        set celle_che_posso_cambiare($pin_name,incL) $incLrtime
					set num_celle_che_posso_cambiare [expr $num_celle_che_posso_cambiare + 1]
                                } else {
                                	set celle_che_posso_cambiare($pin_name,incL) [lsort -unique [lappend $celle_che_posso_cambiare($pin_name,incL) $incLrtime]]
				}
                        }
                #}
			...
			if {[regexp {\/[^Z].*} $pin_name_temp]} {
				...
				if {[info exists celle_che_posso_cambiare($pin_name,ref)]} {
					if {![info exists celle_che_posso_cambiare($pin_name,incH)]} {
						...
						set celle_che_posso_cambiare($pin_name,incH) [lsort -unique [lappend $celle_che_posso_cambiare($pin_name,incH) $incHrtime]]
					}
				}
			}
		#}
	...
	foreach id [array names celle_che_posso_cambiare] {
		[regexp {(U\d+).*} $pin_name_temp void pin_name]
		if {![info exists celle_che_non_posso_cambiare($pin_name_temp,ref)]} {
			...
			if {[regexp {ref} $id]} {
				...
				if {[get_attribute CORE65LPHVT/$newname cell_leakage_power] != ""} {
					...
					set celle_da_cambiare($pin_name,k) [expr $diff / [lindex $celle_che_posso_cambiare($pin_name,incL)  0] - [lindex $celle_che_posso_cambiare($pin_name,incH)  0]]
				}
			}
		} 
	}
